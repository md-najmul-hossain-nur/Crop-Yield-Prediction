...
=============================================================
  ...
  Input : Preprocessing/Data/ (preprocessed CSVs)
...
# (Run rf_gb_train.py first and paste results below,
#  OR the values are auto-filled if rf_gb_results.npy exists)
# ─────────────────────────────────────────────
import os
npy_path = "rf_gb_results.npy"

if os.path.exists(npy_path):
    saved = np.load(npy_path, allow_pickle=True).item()
    ref_reg = saved['reg']
    ref_cls = saved['cls']
    print("\n  ✅ Loaded RF/GB results from rf_gb_results.npy")
else:
    # Fallback: run rf_gb and get results inline
    print("\n  ⚠️  rf_gb_results.npy not found — running RF/GB inline for comparison...")
    from sklearn.ensemble import (
        RandomForestRegressor, RandomForestClassifier,
        HistGradientBoostingRegressor, HistGradientBoostingClassifier
    )

    rf_reg_cmp = RandomForestRegressor(n_estimators=150, max_depth=20,
                                        min_samples_leaf=2, n_jobs=-1, random_state=42)
    rf_reg_cmp.fit(X_tr_r, y_tr_r)
    p = rf_reg_cmp.predict(X_te_r)
    rf_r2 = r2_score(y_te_r, p)
    rf_rmse = np.sqrt(mean_squared_error(y_te_r, p))
    rf_mae  = mean_absolute_error(y_te_r, p)

    gb_reg_cmp = HistGradientBoostingRegressor(max_iter=150, learning_rate=0.05,
                                                max_depth=6, random_state=42)
    gb_reg_cmp.fit(X_tr_r, y_tr_r)
    p = gb_reg_cmp.predict(X_te_r)
    gb_r2   = r2_score(y_te_r, p)
    gb_rmse = np.sqrt(mean_squared_error(y_te_r, p))
    gb_mae  = mean_absolute_error(y_te_r, p)

    rf_cls_cmp = RandomForestClassifier(n_estimators=150, max_depth=20,
                                         min_samples_leaf=2, class_weight='balanced',
                                         n_jobs=-1, random_state=42)
    rf_cls_cmp.fit(X_tr_c, y_tr_c)
    p = rf_cls_cmp.predict(X_te_c)
    rf_acc = accuracy_score(y_te_c, p)
    rf_f1  = f1_score(y_te_c, p, average='weighted', zero_division=0)

    gb_cls_cmp = HistGradientBoostingClassifier(max_iter=150, learning_rate=0.05,
                                                 max_depth=6, random_state=42)
    gb_cls_cmp.fit(X_tr_c, y_tr_c)
    p = gb_cls_cmp.predict(X_te_c)
...
fig.suptitle(
    ...
    'Input: Preprocessed Data  |  Team: Light Seekers | CSE-4889',
