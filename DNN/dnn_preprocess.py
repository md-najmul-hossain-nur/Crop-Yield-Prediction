...
df_full = pd.read_csv('Data/agumnetation/train_augmented.csv')
test    = pd.read_csv('Data/agumnetation/test_original.csv')

print(f"Train (augmented) : {df_full.shape[0]:,} rows")
...
# ─────────────────────────────────────────────
print("\n" + "=" * 60)
print("STEP 1: Train / Val Split (90% / 10%)")
print("=" * 60)
...
# ─────────────────────────────────────────────
# STEP 2: Scale — Regression
# ─────────────────────────────────────────────
print("\n" + "=" * 60)
print("STEP 2: StandardScaler — Regression Features")
print("=" * 60)

scaler_r = StandardScaler()

X_train_r = pd.DataFrame(
    scaler_r.fit_transform(train[REG_FEATURES]),
    columns=REG_FEATURES
)
X_val_r = pd.DataFrame(
    scaler_r.transform(val[REG_FEATURES]),
    columns=REG_FEATURES
)
X_test_r = pd.DataFrame(
    scaler_r.transform(test[REG_FEATURES]),
    columns=REG_FEATURES
)
...
# ─────────────────────────────────────────────
print("\n" + "=" * 60)
print("STEP 3: StandardScaler — Classification Features")
print("=" * 60)

scaler_c = StandardScaler()

X_train_c = pd.DataFrame(
    scaler_c.fit_transform(train[CLS_FEATURES]),
    columns=CLS_FEATURES
)
X_val_c = pd.DataFrame(
    scaler_c.transform(val[CLS_FEATURES]),
    columns=CLS_FEATURES
)
X_test_c = pd.DataFrame(
    scaler_c.transform(test[CLS_FEATURES]),
    columns=CLS_FEATURES
)
...
print("✅ Saved — preprocess/dnn/ folder:")
